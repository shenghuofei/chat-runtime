...
func TestSaveLoadRoundtrip(t *testing.T) {
	...
	if err := s.Save("sess-1", want); err != nil {
		t.Fatalf("保存失败：%v", err)
	}
...
func TestAppendMessage(t *testing.T) {
	...
	for _, m := range msgs {
		if err := s.AppendMessage("sess-2", m); err != nil {
			t.Fatalf("追加消息失败：%v", err)
		}
	}

	got, err := s.LoadLog("sess-2")
	if err != nil {
...
func TestDelete(t *testing.T) {
	...
	if err := s.Save("sess-3", SessionData{SystemPrompt: "x"}); err != nil {
		t.Fatalf("保存失败：%v", err)
	}
	if err := s.AppendMessage("sess-3", provider.Message{Content: "y"}); err != nil {
		t.Fatalf("追加失败：%v", err)
	}
